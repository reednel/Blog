...
In practice, it’s biggest bottleneck is neither time nor space complexity (in runtime). The problem is one of volume of data output. These sequences grow in number and length extraordinarily fast (recall Properties 2.1 and 2.2). At 1 byte/character, a file containing all $B(2,6)$ sequences would be 4GB in size. A file containing all $B(2,7)$ sequences would be 65,000,000,000 GB. Changing the alphabet makes for even more radical growth. A single $B(10,10)$ sequence would be 10GB.
...
For example, take a sequence to be of length 10. An in-shuffle reorders it’s indices as follows: $\\$
...
While applications for de Bruijn sequences extend beyond what's been discussed here, none appear to be too grand or significant. The diversity of applications is perhaps more notable than the applications themselves.

[^1]: [A combinatorial Problem](https://pure.tue.nl/ws/files/4442708/597473.pdf) (1946), Nicolaas Govert de Bruijn.

[^2]: A binary alphabet is often implied with de Bruijn sequences, but what this paper discusses applies for an arbitrary alphabet.
...
[^5]: A **self working** trick refers to one which follows a fixed procedure that does not depend on circumstance or skill.

[^6]: Concerning playing cards, a perfect shuffle (in or out) corresponds to a perfect [Faro shuffle](https://en.wikipedia.org/wiki/Faro_shuffle). In practice, it would take much skill to reliably execute the Faro shuffle perfectly.

[^7]: [A novel codon-based de Bruijn graph algorithm for gene construction from unassembled transcriptomes](https://www.researchgate.net/publication/310461341_A_novel_codon-based_de_Bruijn_graph_algorithm_for_gene_construction_from_unassembled_transcriptomes) (2016).

[^8]: It would be interesting to prove that for all n, there exists a $B(2,n)$ sequence that shuffles.
